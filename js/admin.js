        ...
        const modalContent = `
            <div class="revision-completa">
                ...
                <div class="acciones-ticket">
                    ...
                    <button class="btn-admin-secondary" onclick="descargarSolicitudPDF('${solicitud.id}')">
                        📄 Descargar Documento
                    </button>
                    <button class="btn-admin-secondary" onclick="cerrarModalRevision()">
                        ❌ Cerrar
...
// 3. FUNCIÓN PRINCIPAL PARA DESCARGAR PDF
async function descargarSolicitudPDF(solicitudId) {
    try {
        ...
        showNotificationAdmin('Error al generar el documento PDF', 'error');
    }
}
...
function generarPDFSolicitud(solicitud) {
    ...
    doc.text('Dirección Jurídica', pageWidth / 2, y, { align: 'center' });
    y += 7;
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text(solicitud.usuarios?.departamento || 'Coordinación Administrativa', pageWidth / 2, y, { align: 'center' });
    y += 7;
    
    doc.setFont('helvetica', 'bold');
    const tipoSolicitud = solicitud.recurso_tipo === 'papeleria' ? 'Papelería' : 'Insumos';
    doc.text(`Solicitud de ${tipoSolicitud} y Artículos de Oficina`, pageWidth / 2, y, { align: 'center' });
    y += 15;
    ...
    // Determinar cargo según el tipo de solicitud
    let cargo = 'Partida General';
    if (solicitud.tipo === 'ordinaria') {
        cargo = solicitud.recurso_tipo === 'papeleria' ? 'Token Papelería Ordinario' : 'Token Insumos Ordinario';
    } else if (solicitud.tipo === 'extraordinaria') {
        cargo = 'Token Papelería Extraordinario';
    } else if (solicitud.tipo === 'juntas') {
        cargo = 'Evento/Junta';
    }
    
    doc.text(cargo, margin + 25, y);
    
    // CUADRO DE SALIDA (lado derecho)
    const cuadroX = pageWidth - margin - 50;
    doc.rect(cuadroX, y - 8, 50, 25);
    doc.setFont('helvetica', 'bold');
    doc.text('Salida de', cuadroX + 25, y - 3, { align: 'center' });
    doc.text('Almacén N°', cuadroX + 25, y + 2, { align: 'center' });
    
    // Usar los últimos 3 dígitos del ID como número de salida
    const numeroSalida = solicitud.id.slice(-3).toUpperCase();
    doc.setFontSize(16);
    doc.text(numeroSalida, cuadroX + 25, y + 10, { align: 'center' });
    
    doc.setFontSize(8);
    const fechaSolicitud = new Date(solicitud.fecha_solicitud).toLocaleDateString('es-MX');
    doc.text('Fecha: ' + fechaSolicitud, cuadroX + 25, y + 16, { align: 'center' });
    
    y += 35;
    
    // TABLA DE ITEMS
    doc.setFontSize(10);
    const tableHeaders = ['Descripción', 'Unidad', 'Solicitada', 'Aprobada'];
    const colWidths = [contentWidth * 0.5, contentWidth * 0.2, contentWidth * 0.15, contentWidth * 0.15];
    ...
    solicitud.solicitud_detalles.forEach(detalle => {
        // Obtener nombre del item (polimórfico)
        const nombreItem = detalle.insumos?.nombre || detalle.papeleria?.nombre || 'Item no disponible';
        const unidadMedida = detalle.insumos?.unidad_medida || detalle.papeleria?.unidad_medida || 'pz';
        
        currentX = margin;
        
        // Calcular altura necesaria para la descripción
        const descripcionLines = doc.splitTextToSize(nombreItem, colWidths[0] - 4);
        const rowHeight = Math.max(8, descripcionLines.length * 4);
        ...
        doc.rect(currentX + colWidths[0] + colWidths[1] + colWidths[2], y, colWidths[3], rowHeight);
        
        // Contenido
        doc.text(descripcionLines, currentX + 2, y + 4);
        doc.text(unidadMedida, currentX + colWidths[0] + colWidths[1] / 2, y + rowHeight / 2, { align: 'center' });
        doc.text(detalle.cantidad_solicitada.toString(), currentX + colWidths[0] + colWidths[1] + colWidths[2] / 2, y + rowHeight / 2, { align: 'center' });
        doc.text((detalle.cantidad_aprobada || detalle.cantidad_solicitada).toString(), currentX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] / 2, y + rowHeight / 2, { align: 'center' });
        
        y += rowHeight;
    });
    
    // Agregar filas vacías para completar el documento
    const filasVacias = Math.max(0, 8 - solicitud.solicitud_detalles.length);
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Autorizó', margin + boxWidth / 2, y + 8, { align: 'center' });
    ...
    doc.text('Matrícula', margin + boxWidth * 3 / 4, y + 22, { align: 'center' });
    
    // Línea del nombre
    doc.line(margin + 5, y + 35, margin + boxWidth - 5, y + 35);
    doc.text(solicitud.usuarios?.nombre || 'Nombre del Solicitante', margin + boxWidth / 2, y + 39, { align: 'center' });
    doc.text('Firma', margin + boxWidth - 10, y + 42, { align: 'right' });
    
    // Cuadro "Oficio"
    doc.rect(margin + boxWidth + 10, y, boxWidth, boxHeight);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text('Oficio', margin + boxWidth + 10 + boxWidth / 2, y + 8, { align: 'center' });
    
    // Líneas del oficio
    doc.line(margin + boxWidth + 15, y + 25, margin + boxWidth + 10 + boxWidth - 5, y + 25);
    doc.setFont('helvetica', 'bold');
    doc.text('Fecha', margin + boxWidth + 10 + boxWidth / 2, y + 35, { align: 'center' });
    doc.line(margin + boxWidth + 15, y + 38, margin + boxWidth + 10 + boxWidth - 5, y + 38);
    
    // AGREGAR INFORMACIÓN DEL SISTEMA EN PIE DE PÁGINA
    y = doc.internal.pageSize.getHeight() - 20;
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text(`ID Solicitud: ${solicitud.id}`, margin, y);
    doc.text(`Estado: ${solicitud.estado.toUpperCase()}`, pageWidth - margin, y, { align: 'right' });
    
    if (solicitud.fecha_cerrado) {
        y += 4;
        doc.text(`Cerrada: ${new Date(solicitud.fecha_cerrado).toLocaleDateString('es-MX')}`, margin, y);
    }
    
    // GENERAR NOMBRE DEL ARCHIVO
    const fechaArchivo = new Date().toISOString().split('T')[0];
    const tipoDoc = solicitud.recurso_tipo === 'papeleria' ? 'Papeleria' : 'Insumos';
    const nombreArchivo = `Solicitud_${tipoDoc}_${numeroSalida}_${fechaArchivo}.pdf`;
    
    // DESCARGAR
    doc.save(nombreArchivo);
    
    // Mostrar notificación de éxito
    showNotificationAdmin(`Documento generado: ${nombreArchivo}`, 'success');
    
    console.log('PDF generado exitosamente:', nombreArchivo);
}

// 5. FUNCIÓN PARA OBTENER DATOS DE SOLICITUD (si no existe)
