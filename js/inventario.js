...
/* ===================================
   SISTEMA DE INVENTARIO COMPLETO - inventario.js
   Gestión completa de stock, restock y reportes con PERMISOS
   =================================== */
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        // 5. Proteger funciones según rol
        setTimeout(() => {
            protegerFuncionesParaAdmin();
        }, 1500);
...
function verificarPermisosSuperAdmin() {
    ...
    try {
        ...
        configurarPermisosInventario(user.rol);
...
function configurarPermisosInventario(rol) {
    console.log('🔧 Configurando permisos de inventario para rol:', rol);
    
    if (rol === 'admin') {
        console.log('⚠️ Usuario ADMIN - Modo solo lectura activado');
        setTimeout(() => {
            deshabilitarBotonesParaAdmin();
        }, 1000);
        
        document.addEventListener('inventarioRendered', deshabilitarBotonesParaAdmin);
        
    } else if (rol === 'super_admin') {
        console.log('✅ Usuario SUPER_ADMIN - Acceso completo habilitado');
    }
}

function deshabilitarBotonesParaAdmin() {
    console.log('🔒 Deshabilitando botones para usuario ADMIN...');
    
    // 1. Botones de la tabla
    const botonesEditar = document.querySelectorAll('button[onclick*="editarInsumo"]');
    const botonesRestock = document.querySelectorAll('button[onclick*="abrirModalRestock"]');
    
    botonesEditar.forEach(btn => {
        btn.disabled = true;
        btn.style.opacity = '0.5';
        btn.style.cursor = 'not-allowed';
        btn.title = 'Solo Super Administradores pueden editar insumos';
    });
    
    botonesRestock.forEach(btn => {
        btn.disabled = true;
        btn.style.opacity = '0.5';
        btn.style.cursor = 'not-allowed';
        btn.title = 'Solo Super Administradores pueden modificar stock';
    });
    
    // 2. Botón nuevo insumo
    const btnNuevoInsumo = document.querySelector('button[onclick*="abrirModalNuevoInsumo"]');
    if (btnNuevoInsumo) {
        btnNuevoInsumo.disabled = true;
        btnNuevoInsumo.style.opacity = '0.5';
        btnNuevoInsumo.style.cursor = 'not-allowed';
        btnNuevoInsumo.innerHTML = '🔒 Nuevo Insumo (Solo Super Admin)';
    }
    
    // 3. Mostrar mensaje de permisos
    mostrarMensajePermisos();
}

function mostrarMensajePermisos() {
    if (document.getElementById('mensajePermisos')) return;
    
    const header = document.querySelector('.inventario-header .container');
    if (header) {
        const mensaje = document.createElement('div');
        mensaje.id = 'mensajePermisos';
        ...
        mensaje.innerHTML = `
            ...
            <div>
                ...
                <small>Como Administrador, puedes consultar el inventario pero no modificarlo. Solo Super Administradores pueden editar, crear o modificar stock.</small>
            </div>
        `;
...
function protegerFuncionesParaAdmin() {
    const session = sessionStorage.getItem('currentUser');
    if (!session) return;
    ...
    // Override funciones para admin
    window.editarInsumo = function(insumoId) {
        showNotificationInventario('❌ Solo Super Administradores pueden editar insumos', 'error');
        return false;
    };
    
    window.abrirModalRestock = function(insumoId) {
        showNotificationInventario('❌ Solo Super Administradores pueden modificar stock', 'error');
        return false;
    };
    
    window.abrirModalNuevoInsumo = function() {
        showNotificationInventario('❌ Solo Super Administradores pueden crear insumos', 'error');
        return false;
    };
}
...
async function renderizarInventario() {
            ...
            <tr data-insumo="${insumo.id}" class="${trClass}">
                <td>
                    <div class="insumo-info">
                        <strong>${insumo.nombre}</strong>
                        ${insumo.descripcion ? `<br><small>${insumo.descripcion}</small>` : ''}
                        ${!insumo.activo ? '<br><span style="color: #e74c3c; font-size: 0.8rem;">⚠️ INACTIVO</span>' : ''}
                    </div>
                </td>
                <td>
                    <div class="categoria-badge" style="background: ${categoria?.color || '#657153'}20; color: ${categoria?.color || '#657153'}">
    ...
    // Disparar evento para permisos
    document.dispatchEvent(new Event('inventarioRendered'));
}

function generarBotonesAcciones(insumo) {
    ...
    if (esAdmin) {
        return `
            <div class="acciones-inventario">
                <button class="btn-inventario-action restricted" 
                        onclick="showNotificationInventario('❌ Solo Super Administradores pueden modificar stock', 'error')"
                        title="Solo Super Administradores pueden modificar stock"
                ...
                <button class="btn-inventario-action restricted" 
                        onclick="showNotificationInventario('❌ Solo Super Administradores pueden editar insumos', 'error')"
                        title="Solo Super Administradores pueden editar insumos"
                        style="opacity: 0.5; cursor: not-allowed;">
                    ✏️
                </button>
            </div>
    ...
    return `<div class="acciones-inventario"><span style="color: #999;">Sin permisos</span></div>`;
}
...
function mostrarAlertas(alertas) {
    ...
    alertas.forEach(alerta => {
        const botonAlerta = esAdmin ? 
            `<button class="btn-alerta-action" onclick="showNotificationInventario('❌ Solo Super Administradores pueden modificar stock', 'error')" 
                     style="opacity: 0.6; cursor: not-allowed;">🔒 Reabastecer</button>` :
        ...
        html += `
            <div class="alerta-item">
                ...
                <div class="alerta-info">
                    ...
                    Stock crítico: ${alerta.stock} (mínimo: ${alerta.minimo})
                </div>
                ${botonAlerta}
            </div>
        `;
...
// ===================================
// FUNCIONES DE ESTADO Y VISIBILIDAD
// ===================================
...
function getVisibilidadLabel(accesoTipo) {
    ...
    return labels[accesoTipo] || '👥 Todos';
}
...
function filtrarInventario() {
    ...
    if (filtroVisibilidad) {
        inventarioFiltrado = inventarioFiltrado.filter(item => 
            (item.acceso_tipo || 'todos') === filtroVisibilidad
        );
    }
    
    // Temporal: guardar datos originales y renderizar filtrados
    const originalData = inventarioData;
    inventarioData = inventarioFiltrado;
    renderizarInventario();
    inventarioData = originalData;
}
...
function actualizarInfoInsumo() {
    ...
    document.getElementById('stockMinimo').textContent = insumo.cantidad_warning;
    document.getElementById('unidadMedida').textContent = insumo.unidad_medida;
    document.getElementById('visibilidadActual').textContent = getVisibilidadLabel(insumo.acceso_tipo || 'todos');
    document.getElementById('insumoInfoCard').style.display = 'block';
...
async function confirmarRestock() {
    try {
        ...
        if (!insumoId || cantidad <= 0 || !tipoMovimiento) {
        ...
        // 1. Actualizar stock en insumos
        const { error: errorStock } = await supabaseInventario
            .from('insumos')
            .update({ 
                stock_actual: stockNuevo,
                updated_at: new Date().toISOString()
        ...
        // 2. Registrar movimiento
        const { error: errorMovimiento } = await supabaseInventario
            .from('inventario_movimientos')
            .insert({
...
async function editarInsumo(insumoId) {
    ...
    try {
        ...
        if (!document.getElementById('editarInsumoModal')) {
            crearModalEditarInsumo();
        }
        
        await cargarDatosEdicionInsumo(insumo);
        document.getElementById('editarInsumoModal').style.display = 'flex';
        document.body.style.overflow = 'hidden';
        
    } catch (error) {
        console.error('Error editando insumo:', error);
        showNotificationInventario('Error al abrir edición de insumo', 'error');
    }
}

function crearModalEditarInsumo() {
    const modalHTML = `
        <div class="modal-overlay" id="editarInsumoModal" style="display: none;">
            <div class="modal-content restock-modal">
                <div class="modal-header">
                    <h3>✏️ Editar Insumo</h3>
                    <button class="modal-close" onclick="cerrarModalEditarInsumo()">&times;</button>
                </div>
                <div class="modal-body">
                    <form id="editarInsumoForm" class="restock-form">
                        <input type="hidden" id="editarInsumoId">

                        <div class="form-group">
                            <label for="editarNombre">Nombre del insumo *</label>
                            <input type="text" id="editarNombre" required>
                        </div>

                        <div class="form-group">
                            <label for="editarDescripcion">Descripción</label>
                            <textarea id="editarDescripcion" rows="2"></textarea>
                        </div>
                        ...
                        <div class="field-row">
                            ...
                            <div class="form-group">
                                <label for="editarUnidad">Unidad de medida *</label>
                                <select id="editarUnidad" required>
                        ...
                        <div class="field-row">
                            <div class="form-group">
                                <label for="editarStockMinimo">Stock mínimo *</label>
                                <input type="number" id="editarStockMinimo" min="1" required>
                            </div>

                            <div class="form-group">
                                <label for="editarVisibilidad">Visibilidad *</label>
                                <select id="editarVisibilidad" required>
                                    <option value="todos">👥 Todos los usuarios</option>
                                    <option value="solo_direccion">🏢 Solo Dirección Jurídica</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="checkbox-label">
                                <input type="checkbox" id="editarActivo" checked>
                                <span class="checkmark"></span>
                                Insumo activo (visible en solicitudes)
                            </label>
                        </div>
                ...
                <div class="modal-actions">
                    <button type="button" class="btn-admin-secondary" onclick="cerrarModalEditarInsumo()">
                        Cancelar
                    </button>
                    <button type="button" class="btn-admin-primary" onclick="guardarEdicionInsumo()" id="btnGuardarEdicion">
                        💾 Guardar Cambios
                    </button>
                </div>
            </div>
...
async function cargarDatosEdicionInsumo(insumo) {
    await cargarCategoriasEnSelect('editarCategoria');
    ...
    document.getElementById('editarStockMinimo').value = insumo.cantidad_warning;
    document.getElementById('editarVisibilidad').value = insumo.acceso_tipo || 'todos';
    document.getElementById('editarActivo').checked = insumo.activo;
}
...
async function guardarEdicionInsumo() {
    try {
        ...
        const descripcion = document.getElementById('editarDescripcion').value.trim();
        const categoriaId = document.getElementById('editarCategoria').value;
        const unidadMedida = document.getElementById('editarUnidad').value;
        const stockMinimo = parseInt(document.getElementById('editarStockMinimo').value);
        const visibilidad = document.getElementById('editarVisibilidad').value;
        const activo = document.getElementById('editarActivo').checked;
        
        if (!nombre || !categoriaId || !unidadMedida || !stockMinimo) {
        ...
        showNotificationInventario(`Error: ${error.message}`, 'error');
...
function cerrarModalEditarInsumo() {
    const modal = document.getElementById('editarInsumoModal');
    if (modal) {
...
async function confirmarNuevoInsumo() {
    try {
        ...
        const descripcion = document.getElementById('descripcionInsumo').value.trim();
        const categoriaId = document.getElementById('categoriaInsumo').value;
        const unidadMedida = document.getElementById('unidadInsumo').value;
        const stockMinimo = parseInt(document.getElementById('stockMinimoInsumo').value);
        const stockInicial = parseInt(document.getElementById('stockInicialInsumo').value);
        const visibilidad = document.getElementById('visibilidadInsumo').value;
        
        if (!nombre || !categoriaId || !unidadMedida || stockMinimo < 1) {
            throw new Error('Todos los campos obligatorios deben estar llenos');
        }
...
async function cargarMovimientosRecientes() {
    try {
        ...
        renderizarMovimientosRecientes(movimientos || []);
...
function renderizarMovimientosRecientes(movimientos) {
    ...
    if (movimientos.length === 0) {
        container.innerHTML = '<p class="text-center">No hay movimientos recientes</p>';
        return;
    }
    
    let html = '<div class="movimientos-lista">';
    ...
    movimientos.forEach(mov => {
        ...
        html += `
            <div class="movimiento-item">
                <div class="movimiento-info">
                    <div class="movimiento-tipo">${tipoIcon} ${getTipoMovimientoLabel(mov.tipo_movimiento)}</div>
                    <div class="movimiento-insumo">${mov.insumos?.nombre || 'N/A'}</div>
                    <div class="movimiento-fecha">${new Date(mov.fecha).toLocaleDateString()}</div>
                </div>
                <div class="movimiento-cantidad ${mov.cantidad > 0 ? 'positivo' : 'negativo'}">
                    ${signo}${cantidad}
                </div>
            </div>
        `;
...
function showNotificationInventario(message, type = 'info') {
    const notification = document.createElement('div');
    notification.className = `notification-inventario notification-${type}`;
    notification.style.cssText = `
        position: fixed;
...
function mostrarLoadingInventario(mostrar) {
    const loading = document.getElementById('loadingInventario');
    const tabla = document.getElementById('tablaInventario');
    const sinInventario = document.getElementById('sinInventario');
    
    if (loading) loading.style.display = mostrar ? 'block' : 'none';
    if (tabla) tabla.style.display = mostrar ? 'none' : 'block';
...
function mostrarError(mensaje) {
    ...
    if (sinInventario) {
        ...
        sinInventario.innerHTML = `
            <h3>❌ Error</h3>
            <p>${mensaje}</p>
            <button class="btn-admin-primary" onclick="cargarDatosInventario()">
                🔄 Reintentar
            </button>
        `;
    }
...
function cargarHeaderAdmin() {
    fetch('/includes/headerAdmin.html')
        .then(response => response.text())
        .then(html => {
            document.getElementById('header-contain').innerHTML = html;
            setTimeout(inicializarHeaderAdmin, 200);
        })
        .catch(error => {
...
function inicializarHeaderAdmin() {
    ...
    if (session) {
        try {
            ...
            console.log('Header de inventario inicializado para:', user.nombre);
...
function configurarEventListeners() {
    ...
    if (filtroEstadoStock) filtroEstadoStock.addEventListener('change', filtrarInventario);
    if (filtroVisibilidad) filtroVisibilidad.addEventListener('change', filtrarInventario);
    
    const cantidadAgregar = document.getElementById('cantidadAgregar');
    if (cantidadAgregar) cantidadAgregar.addEventListener('input', calcularNuevoStock);
    
    const insumoSelect = document.getElementById('insumoSelect');
    if (insumoSelect) insumoSelect.addEventListener('change', actualizarInfoInsumo);
    
    console.log('✅ Event listeners configurados para inventario');
}
...
async function actualizarInventario() {
    await cargarDatosInventario();
    showNotificationInventario('Inventario actualizado', 'success');
}

function exportarInventario() {
    try {
        let csv = 'Nombre,Descripción,Categoría,Stock Actual,Stock Mínimo,Unidad,Visibilidad,Estado\n';
        
        inventarioData.forEach(insumo => {
            const categoria = insumo.categorias_insumos?.nombre || 'Sin categoría';
            const visibilidad = getVisibilidadLabel(insumo.acceso_tipo || 'todos');
            const estado = insumo.activo ? 'Activo' : 'Inactivo';
            
            csv += `"${insumo.nombre}","${insumo.descripcion || ''}","${categoria}",${insumo.stock_actual},${insumo.cantidad_warning},"${insumo.unidad_medida}","${visibilidad}","${estado}"\n`;
        });
        ...
        showNotificationInventario('Error al exportar inventario', 'error');
    }
}

async function verHistorialInsumo(insumoId) {
    try {
        const insumo = inventarioData.find(i => i.id == insumoId);
        if (!insumo) return;
        
        showNotificationInventario(`Cargando historial de ${insumo.nombre}...`, 'info');
        
        setTimeout(() => {
            showNotificationInventario(`Historial de ${insumo.nombre} - Función en desarrollo`, 'info');
        }, 1000);
...
console.log('📦 Inventario.js cargado completamente con sistema de permisos');